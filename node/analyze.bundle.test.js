...
function parseAepx(aepxPath) {
  return new Promise((resolve, reject) => {
    ...
    const result = {
      bdataBlobs: [],
      // { compName, layerName, raw (hex string), decoded (utf8) }
      layerSources: []
      // { compName, layerName, sourceName }
    };
...
async function main() {
  ...
  const externalCompMap = /* @__PURE__ */ new Map();
  if (externalCompLayers) {
    for (const ec of externalCompLayers) {
      if (selectedIds.has(ec.compId)) {
        warnings.push(
          'Snapshot conflict: comp "' + ec.compName + '" (#' + ec.compId + ") appears in both selectedComps and externalCompLayers. External layer data for this comp will be ignored."
        );
      }
    }
  }
  ...
  const globalBdataStrings = /* @__PURE__ */ new Set();
  ...
  for (const item of allItems) {
    ...
    if (reachable.has(item.id))
      continue;
    if (forceKeep.has(item.id))
      continue;
    if (item.name === "_LIMBO_DeepClean")
      continue;
  ...
  for (const entry of itemsToMoveToLimbo) {
    if (entry.filePath && fileExists(entry.filePath)) {
      try {
        savedBytes += fs.statSync(normPath(entry.filePath)).size;
      } catch (e) {
      }
    }
  }
